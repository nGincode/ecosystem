                                                ...
                                                return <div className={val.full ? "col-12 col-md-12" : "col-12 col-md-6"} key={i}>
                                                    {
                                                        val.type === 'text' || val.type === 'number' || val.type === 'email' || val.type === 'date' ?
                                                            <div className="mb-24">
                                                                <Input required={val.required} label={convertCamelCase(val.name)} variant="standard" className="border-b-1" type={val.type} defaultValue={
                                                                    val.type === 'date' ?
                                                                        dataEdit?.[val.name] ?
                                                                            moment(dataEdit?.[val.name], 'DD/MM/YYYY').format('YYYY-MM-DD') :
                                                                            dataEdit?.[val.name] ? dataEdit?.[val.name] : '' :
                                                                        dataEdit?.[val.name] ? dataEdit?.[val.name] : ''} name={val.name} id={`${val.id}Edit`} />
                                                            </div>
                                                            : val.type === 'group' ?
                                                                <div className="mb-24">
                                                                    {val.label || val.name ?
