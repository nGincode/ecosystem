            ...
            if (val == 'uuid' || val == 'addressJson' || val == 'itemJson') { } else {
                if (val === 'img') {
                    array.push({
                        ...
                        cell: ({ row }: any) => {
                            if (row.original.img) {
                                return <div className="avatar-item avatar-lg d-flex align-items-center justify-content-center bg-primary-4 hp-bg-dark-primary text-primary hp-text-color-dark-0 rounded-circle"><Image src={row.original.img} className=" object-cover rounded-full w-12 h-12" alt={row.original.name} />
                                </div>
                            } else {
