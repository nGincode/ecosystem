...
export default function StockData({ userData, setuserData }: any) {
    ...
    const convertToPdfBCA = async (textList: any) => {
        const convertToExcel = async (data: any) => {
            ...
            XLSX.writeFile(workbook, "BCA DEBIT KREDIT " + moment(data1?.[0].tanggal.split('/')[1], 'MM').format('MMMM').toUpperCase() + ".xlsx", { compression: true });
        }
    ...
    const convertToPdfMANDIRI = async (textList: any) => {
        ...
        const convertToExcel = async (data: any) => {
            ...
            XLSX.writeFile(workbook, "MANDIRI DEBIT KREDIT " + moment(data?.[0].TANGGAL.split('/')[1], 'MM').format('MMMM').toUpperCase() + ".xlsx", { compression: true });
        }
                ...
                if (start) {
                    if (mp1.str.includes("Page") && mp1.str.includes("of")) {
                        ...
                        if (mp1.str.split('/').length == 3) {
                            tgl += 1;
                            if (tgl == 1) {
                                let cek = array[k + 6]?.str;
                                return {
                                    date: mp1.str,
                                    // desc: array[k + 4]?.str + array[k + 5]?.str + cek,
                                    time: array[k + 1].str,
