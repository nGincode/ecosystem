...
export default function StockData({ userData, setuserData }: any) {
    ...
    const convertToExcelBCA = async (data: any) => {
    ...
    const convertToExcelCAPITAL = async (data: any) => {

        const ws1 = XLSX.utils.json_to_sheet(data);
        const workbook = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(workbook, ws1, "Data");

        XLSX.writeFile(workbook, "SINARMAS DEBIT KREDIT.xlsx", { compression: true });
    }
    ...
    const convertToExcelMANDIRI = async (data: any) => {

        const ws1 = XLSX.utils.json_to_sheet(data);
        const workbook = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(workbook, ws1, "Data");

        XLSX.writeFile(workbook, "MANDIRI DEBIT KREDIT.xlsx", { compression: true });
    }
    ...
    const convertToExcelDANAMON = async (data: any) => {

        const ws1 = XLSX.utils.json_to_sheet(data);
        const workbook = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(workbook, ws1, "Data");

        XLSX.writeFile(workbook, "DANAMON DEBIT KREDIT.xlsx", { compression: true });
    }
    const convertToPdfDANAMON = async (textList: any) => {
        ...
        await textList.map((mp: any) => {
                ...
                if (mp1.str == "Debit") {
                    ...
                    if (mp1.str == "Total") {
                        ...
                        if (point > 4) {
                            point1 += 1;
                            if (point1 > 1) {
                                if (moment(mp1.str, 'DD MMM YYYY').format('DD/MM/DD') !== 'Invalid date' && mp1.str.length == 11) {
            ...
            step3.map((mp: any) => {
                result.push({
                    "Date": mp['Date'],
                    "Transaction Branch": mp['Transaction Branch'],
                    "Reference Number": mp['Reference Number'],
                    "Description": mp['Description'],
                    "Debit": numeral(mp['Debit']).value(),
                    "Credit": numeral(mp['Credit']).value(),
                    "Balance": numeral(mp['Balance']).value(),
                })
            })
        })
        return result
    ...
    const convertToExcelBRI = async (data: any) => {

        const ws1 = XLSX.utils.json_to_sheet(data);
        const workbook = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(workbook, ws1, "Data");

        XLSX.writeFile(workbook, "BRI DEBIT KREDIT.xlsx", { compression: true });
    }
    ...
    const convertToExcelSINARMAS = async (data: any) => {
    ...
    const convertToPdfSINARMAS = async (textList: any) => {
        ...
        await textList.map((mp: any) => {
            ...
            step2.map((mp3: any) => {
                result.push({
                    'Posting Date': mp3['Posting Date'],
                    'Description': mp3['Description'],
                    'Reference': mp3['Reference'],
                    'Value Date': mp3['Value Date'],
                    'Detail': mp3['Detail'],
                    'Debit': mp3['Debit'],
                    'Credit': mp3['Credit'],
                    'Balance': mp3['Balance'],
                ...
                if ('application/pdf' === files[index].type) {
                    if (type == 'BCA') {
                        ...
                        if (index == files.length - 1) {
                            if (arrayData.length) {
                                console.log(_.sortBy(arrayData, ['TAHUN']));
                                arrayData = [];
                            } else {
                                toast.error(`Format tidak valid`);
                            }
                        ...
                        if (index == files.length - 1) {
                            if (arrayData.length) {
                                convertToExcelBCA(_.sortBy(arrayData, ['TAHUN']));
                                arrayData = [];
                            }
                        ...
                        if (index == files.length - 1) {
                            if (arrayData.length) {
                                convertToExcelMANDIRI(_.sortBy(arrayData, ['TANGGAL']));
                                arrayData = [];
                            } else {
                                toast.error(`Format tidak valid`);
                            }
                        ...
                        if (index == files.length - 1) {
                            if (arrayData.length) {
                                convertToExcelDANAMON(_.sortBy(arrayData, ['Date']));
                                arrayData = [];
                            } else {
                                toast.error(`Format tidak valid`);
                            }
                        ...
                        if (index == files.length - 1) {
                            if (arrayData.length) {
                                convertToExcelBRI(_.sortBy(arrayData, ['Tanggal Transaksi']));
                                arrayData = [];
                            } else {
                                toast.error(`Format tidak valid`);
                            }
                        ...
                        if (index == files.length - 1) {
                            if (arrayData.length) {
                                convertToExcelSINARMAS(_.sortBy(arrayData, ['Posting Date']));
                                arrayData = [];
                            } else {
                                toast.error(`Format tidak valid`);
                            }
                        ...
                        const pageList = await Promise.all(Array.from({ length: pdf.numPages }, (_, i) => pdf.getPage(i + 1)));
                        const textList = await Promise.all(pageList.map((p) => p.getTextContent()));
                        convertToPdfCAPITAL(textList);
                        // if (index == files.length - 1) {
                        //     convertToExcelCAPITAL(CAPITALData);
                        //     CAPITALData = [];
                        // }
                    }
                } else {
                        ...
                        <div className="card-body px-0 text-center flex justify-center flex-wrap">
                            ...
                            <div className="mb-5">
                                <label htmlFor="fileConvert2">
                                    ...
                                    <input type="file" id="fileConvert2" name="file" multiple accept="application/pdf" style={{ display: "none" }} onChange={(val: any) => pdfRead(val, 'BCA')} />
                                </label>
                                <div className="text-center mt-1">
                            ...
                            <div className="mb-5">
                                ...
                                <div className="text-center mt-1">
                                    <Link href="/format/Format Bank Capital Statement.pdf">
                                        Example Format
                                    </Link>
                                </div>
                            </div>
