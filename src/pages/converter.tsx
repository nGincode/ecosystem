...
import _, { isNumber, trim } from 'lodash';
...
export default function StockData({ userData, setuserData }: any) {
    ...
    const convertToPdfBSI = async (textList: any) => {
        let res: any = [];
        let start = 0;
        await textList.map((mp: any, key: any) => {
            let step1: any = [];
            mp.items.map((mp1: any, k: any, array: any) => {
                if (mp1.str == 'Keterangan') {
                    start += 1;
                }
                if (start && isNumber(Number(mp1.str)) && mp1.str.length == 8) {
                    start += 1;
                }

                if (start > 1) {
                    if (isNumber(Number(mp1.str)) && mp1.str.length == 8) {
                        step1.push({ tanggal: moment(mp1.str, "YYYYMMDD").format('DD/MM/YYYY') });
                    } else if (mp1.str == "D") {
                        let ket = "";
                        ...
                        if (array[k + 5]?.str && isNaN(Number(array[k + 5]?.str))) {
                            ket += " " + array[k + 5].str;
                        }

                        if (array[k + 6]?.str && isNaN(Number(array[k + 6]?.str))) {
                            ket += " " + array[k + 6].str;
                        }
                        ...
                        saldo = split?.[0] ?? "";
                        ket = split.join(" ");
                        step1[step1.length - 1] = { ...step1[step1.length - 1], ...{ debit: numeral(array[k + 2].str.replaceAll("-", "")).value(), kredit: "", saldo: numeral(saldo).value(), keterangan: trim(ket.replace(saldo, "")) } }
                    } else if (mp1.str == "K") {
                        let ket = "";
                        ...
                        saldo = split?.[0] ?? "";
                        ket = split.join(" ");
                        step1[step1.length - 1] = { ...step1[step1.length - 1], ...{ kredit: numeral(array[k + 2].str.replaceAll("-", "")).value(), debit: "", saldo: numeral(saldo).value(), keterangan: trim(ket.replace(saldo, "")) } }
                    }
                }
        ...
        let result = res.map((mp3: any) => {
            return {
                ...
                KREDIT: mp3.kredit,
                SALDO: mp3.saldo,
                TAHUN: moment(mp3.tanggal, "DD/MM/YYYY").format('YYYY'),
            }
        })
    ...
    const convertToPdfBSINEW = async (textList: any) => {
        ...
        await textList.map((mp: any, key: any) => {
            let step1: any = [];
            mp.items.map((mp1: any, k: any, array: any) => {
                if (mp1.str.length == 19 && moment(mp1.str, "YYYY-MM-DD HH:mm:ss").format('YYYY-MM-DD') !== 'Invalid date') {
                    start = 1;
                }
                if (mp1.str.indexOf('Page ') !== -1 || mp1.str.indexOf('/') !== -1) {
                    start = 0;
                }
                if (start && mp1.str == "Date" && array[k + 2]?.str == "FT Number") {
                    start = 0;
                }

                if (start) {
                    if (mp1.str.length == 19 && moment(mp1.str, "YYYY-MM-DD HH:mm:ss").format('YYYY-MM-DD') !== 'Invalid date') {
                        step1.push({ tanggal: mp1.str })
                    } else {
                        step1.push(mp1.str)
                    }
                }
            ...
            step1.map((mp2: any) => {
                if (mp2?.tanggal) {
                    if (step2row?.tanggal) {
                        step2.push(step2row);
                        step2row = {}
                        step2row = { tanggal: mp2.tanggal }
                    } else {
                        step2row = { tanggal: mp2.tanggal }
                    }
                } else {
                    if (step2row.ket) {
                        step2row = { ...step2row, ket: [...step2row.ket, mp2] }
                    } else {
                        step2row = { ...step2row, ket: [mp2] }
                    }
                }
            })
            step2.push(step2row);
                ...
                mp3.ket?.map((m: any, ky: any, arr: any) => {
                    ...
                    if (ky > 1) {
                        if (!idr) {
                            ...
                            if (m === "IDR") {
                                if (mp3.ket.indexOf("CR") !== -1) {
                                    array.kredit += arr[mp3.ket.indexOf("CR") - 2];
                                    array.saldo += arr[mp3.ket.indexOf("CR") + 2];
                                } else if (mp3.ket.indexOf("DB") !== -1) {
                                    array.debit += arr[mp3.ket.indexOf("DB") - 2];
                                    array.saldo += arr[mp3.ket.indexOf("DB") + 2];
                                }
                            }
                        }
                    }
        ...
        let result = res.map((mp4: any) => {
            return {
                TANGGAL: moment(mp4.tanggal, "YYYY-MM-DD HH:mm:ss").format('DD/MM/YYYY'),
                KETERANGAN: trim(mp4.keterangan),
                DEBIT: numeral(mp4.debit).value(),
                KREDIT: numeral(mp4.kredit).value(),
                SALDO: numeral(mp4.saldo).value(),
                TAHUN: moment(mp4.tanggal, "YYYY-MM-DD HH:mm:ss").format('YYYY'),
            }
        })
            ...
            for (let index = 0; index < files.length; index++) {
                if ('application/pdf' === files[index].type) {
                    if (type == 'BCA_GIRO') {
                        ...
                        const pageList = await Promise.all(Array.from({ length: pdf.numPages }, (_, i) => pdf.getPage(i + 1)));
                        const textList = await Promise.all(pageList.map((p) => p.getTextContent()));
                        arrayData = [...arrayData, ...await convertToPdfBSI(textList)];
                        if (index == files.length - 1) {
                            if (arrayData.length) {
                                convertToExcel(_.sortBy(arrayData, ['TAHUN']), 'BSI DEBIT KREDIT');
                                arrayData = [];
                            } else {
                                toast.error(`Format tidak valid`);
                            }
                        ...
                        const pageList = await Promise.all(Array.from({ length: pdf.numPages }, (_, i) => pdf.getPage(i + 1)));
                        const textList = await Promise.all(pageList.map((p) => p.getTextContent()));
                        arrayData = [...arrayData, ...await convertToPdfBSINEW(textList)];
                        if (index == files.length - 1) {
                            if (arrayData.length) {
                                convertToExcel(_.sortBy(arrayData, ['TAHUN']), 'BSI DEBIT KREDIT');
                                arrayData = [];
                        ...
                        <div className="card-body px-0 text-center flex justify-center flex-wrap">
                            ...
                            <div className="mb-5">
                                <label htmlFor="fileConvert10">
                                    <span className="dropdown-item text-center rounded-lg  cursor-pointer" aria-hidden="true">Convert BANK BSI
                                        <div className="flex justify-center mt-3">
                                            <svg xmlns="http://www.w3.org/2000/svg" height="50px" width="50px" version="1.1" id="Layer_1" viewBox="0 0 303.188 303.188" >
                                                <g>
                                                    <polygon style={{ fill: '#E8E8E8' }} points="219.821,0 32.842,0 32.842,303.188 270.346,303.188 270.346,50.525  " />
                                    ...
                                    <input type="file" id="fileConvert10" name="file" multiple accept="application/pdf" style={{ display: "none" }} onChange={(val: any) => pdfRead(val, 'BSI')} />
                                </label>
                                <div className="text-center mt-1">
                                    <Link href="/format/Format BSI.pdf">
                                        Example Format
                                    </Link>
                                </div>
                            </div>
                            ...
                            <div className="mb-5">
                                <label htmlFor="fileConvert11">
                                    <span className="dropdown-item text-center rounded-lg  cursor-pointer" aria-hidden="true">Convert BANK BSI NEW
                                        <div className="flex justify-center mt-3">
                                            <svg xmlns="http://www.w3.org/2000/svg" height="50px" width="50px" version="1.1" id="Layer_1" viewBox="0 0 303.188 303.188" >
                                                <g>
                                                    <polygon style={{ fill: '#E8E8E8' }} points="219.821,0 32.842,0 32.842,303.188 270.346,303.188 270.346,50.525  " />
                                    ...
                                    <input type="file" id="fileConvert11" name="file" multiple accept="application/pdf" style={{ display: "none" }} onChange={(val: any) => pdfRead(val, 'BSINEW')} />
                                </label>
                                <div className="text-center mt-1">
                                    <Link href="/format/Format BSI NEW.pdf">
                                        Example Format
                                    </Link>
                                </div>
                            </div>
                        ...
                        <div className="card-body px-0 text-center flex justify-center flex-wrap">
                            ...
                            <div className="mb-5 ">
                                <label htmlFor="fileConvert31">
                                    ...
                                    <input type="file" id="fileConvert31" name="file" accept="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" style={{ display: "none" }} onChange={(val: any) => importFileSPTTahunan3(val)} />          </label>
                                <div className="text-center mt-1">
                            ...
                            <div className="mb-5 ">
                                <label htmlFor="fileConvert32">
                                    ...
                                    <input type="file" id="fileConvert32" name="file" accept="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" style={{ display: "none" }} onChange={(val: any) => importFileSPTTahunan1A(val)} />          </label>
                                <div className="text-center mt-1">
